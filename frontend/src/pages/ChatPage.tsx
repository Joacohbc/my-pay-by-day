...
export function ChatPage() {
  ...
  const isChatListVisible = !showTasks && showChatList;
  const activeTasksCount =
    tasks?.filter((task) => task.status === 'RUNNING' || task.actions?.some((a) => a.status === 'PENDING_APPROVAL'))
      .length ?? 0;

  const lastMessage = messages.at(-1);
  const showContinueCard = !showTasks && !isPending && lastMessage?.role === 'assistant' && lastMessage.stoppedByStepLimit;
