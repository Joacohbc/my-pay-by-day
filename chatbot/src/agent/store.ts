...
export const agentStore = {
  create(input: {
    ...
    db()
      .prepare(
        `INSERT INTO agent_task (id, user_instruction, execution_mode, status, progress, lang, timezone, currency, request_id, title, created_at, updated_at)
         VALUES (?, ?, ?, 'PENDING', 0, ?, ?, ?, ?, ?, ?, ?)`,
      )
      .run(id, input.instruction, input.executionMode, input.lang, input.timezone, input.currency, input.requestId, input.title ?? null, now, now);
