...
* **Cache invalidation map:** `frontend/src/lib/chat/toolInvalidation.ts` must be updated so every tool name is classified. Mutating tools go in `TOOL_DOMAINS`, mapped to the `CacheDomain`s they affect; read-only tools go in `READ_TOOL_NAMES`. An unmapped tool falls back to a broad finance invalidation — correct but noisy — so classify it explicitly.
  * **CRITICAL:** When dealing with tool invocations from Vercel AI SDK streams, ensure you extract `toolName` and `state` properly using helper functions like `stateOf` or `toolNameOf`, since the SDK might nest these properties inside `part.toolInvocation` (e.g., `part.toolInvocation?.state`) rather than at the top level of the `part` object. Failing to do this causes invalidation logic to silently skip, leading to stale lists (e.g. `EventsPage` not updating after a tool completes).
