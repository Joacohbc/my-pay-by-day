...
* **OpenAPI annotations on every endpoint:** All public REST methods must carry `@Operation` with a summary, and `@APIResponse`/`@APIResponses` for every possible HTTP status code returned. This is not just documentation: when an endpoint has no `@APIResponse`, `quarkus-smallrye-openapi` *guesses* a status code from the return type instead of leaving it undocumented, and that guess can silently disagree with what the method actually returns (e.g. it inferred `201 Created` for a `RestResponse<Void>` endpoint that always returns `204 No Content`) — corrupting the generated `openapi.json` and, downstream, the chatbot's typed API client (`chatbot/src/backend/schema.d.ts`). `ResourceOpenApiArchTest` (`mypaybyday/src/test/java/com/mypaybyday/architecture/`) enforces this with ArchUnit: it fails `./mvnw test` if any JAX-RS endpoint in `resource/` is missing `@Operation` or `@APIResponse`/`@APIResponses`.
...
* **The Markdown rendering is the compatibility layer.** `EmailFileFormat.renderMarkdown` is persisted as the file's `markdownContent` at ingest, so every reader that already understands "a file converted to Markdown" — the AI attachment pipeline, the generic preview — reads an email with no knowledge of this format. Emails are the one file type whose Markdown is produced by the application rather than by the sidecar, which is why `FileService.storeConvertedFile` exists. Because that rendering is derived data, `FileService.getMarkdownContent` re-renders it for email files on every read instead of trusting the stored copy — a fix to how an email reads therefore applies to the emails already stored, not only to the next upload.
* **The date must carry its offset.** `messageDate` is received as an ISO-8601 **string** and normalised to server wall-clock time by `EmailFileFormat.parseMessageDate`: a value with an offset (what the email's own `Date:` header has) is converted, one without is taken as already being server time. The Markdown header then states the offset (`2026-07-31 09:05 UTC+00:00`), because the AI reads that text grounded in the *user's* timezone — an unlabelled server-time header is read as local time and reports the wrong day.
...
4. **An "empty" dashboard variable is not inert, and its neutral value depends on the operator.** A textbox left blank interpolates literally, and LogQL anchors label-filter regexes (`^(?:)$`) — so `| ip =~ ``` matches only the lines *without* an `ip`, silently inverting every panel that uses it. Label filters (`| x =~ `$v``) need `.*` as their default; line filters (`|= `$v``) are substring matches where `.*` would search for two literal characters and `""` is the correct match-all. Check which one a panel uses before touching its variable.
5. **`count_over_time` counts log lines, not events.** `source` is an MDC field stamped on *every* line of a request, and explicitly on heartbeats and scheduled jobs, so a `sum by (source)` with no further filter reports the once-a-minute heartbeat as the busiest client in the system. Constrain to the line that represents the thing being counted — `| resource != ``` for a backend request.
6. **A zero must not look like a broken pipeline.** An empty LogQL result renders as "No data", which on a green stat panel is indistinguishable from a healthy zero *and* from Alloy having stopped shipping. Error and failure stats end in `or vector(0)`.
7. **Telemetry must never evict errors.** Client telemetry keeps its own in-memory buffer (`rumReporter.ts`), separate from the capped, IndexedDB-persisted error buffer in `errorLogStore`.
