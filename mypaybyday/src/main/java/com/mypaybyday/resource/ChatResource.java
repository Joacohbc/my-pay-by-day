...
import com.mypaybyday.service.agent.ChatService;
...
public class ChatResource {
        ...
        try {
            ...
            if (hasImages) {
                ...
                for (FileUpload img : images) {
                    ...
                    extractedImages.add(Image.builder().base64Data(base64Data).mimeType(mimeType).build());
                }
                aiResponse = chatService.processImages(chatId, extractedImages, message);
            } else {
                log.infof("Chat request [chatId=%s]", chatId);
                aiResponse = chatService.processText(chatId, message);
            }
    ...
    public Response trimMemory(
        ...
        chatService.trimMemory(chatId, request.getTextToMatch());
