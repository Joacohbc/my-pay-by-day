...
public record PaymentPlanDto(
	...
	LocalDate endDate,
	@Schema(description = "Last date the plan covers, derived from the cuota count when no end date was given. Null when the plan is open-ended.") LocalDate scheduleEndDate,
	LocalDate nextDueDate,
	boolean isAutomated,
	boolean autoCreateDraft,
	@Schema(description = "Supplies the origin and destination nodes of every event this plan generates.") TemplateDto template,
	...
	public static PaymentPlanDto from(PaymentPlanEntity entity) {
		...
		return new PaymentPlanDto(
			...
			entity.startDate,
			entity.endDate,
			entity.scheduleEndDate(),
			entity.nextDueDate,
			entity.isAutomated,
			entity.autoCreateDraft,
			entity.template != null ? TemplateDto.from(entity.template) : null,
